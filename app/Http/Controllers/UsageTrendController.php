...
class UsageTrendController extends Controller  {

    public function getByLines( $param){

        if($param == 'VnS'){
            $byLines = DB::table('lines_connect')
                ->select(DB::raw("phone_lines, count(phone_lines) as count, MONTHNAME(date_period) as period, sum(duration) AS total"))
        ...
        foreach($byLines as $line){
            ...
            if( !isset($LinesPerMonth[$line->phone_lines])){
                ...
                if(!isset($LinesPerMonth[$line->phone_lines][$line->period])){
                    $LinesPerMonth[$line->phone_lines][$line->period] = $line->total;
                }
                else{
                    $LinesPerMonth[$line->phone_lines][$line->period]+=$line->total;
    ...
    public function getByCallType(){
        ...
        $byCallType = DB::table('lines_connect')
            ...
            ->select(DB::raw("lines_connect.id, lines_connect.tariff_group, tariff_code.code_desc, count(lines_connect.tariff_group),
                                MONTHNAME(lines_connect.date_period) as period,sum(lines_connect.duration) AS total"))
        ...
        foreach($byCallType as $tariff_group){
            ...
            if( !isset($tariffPerMonth[$tariff_group->tariff_group])){
                ...
                if(!isset($tariffPerMonth[$tariff_group->tariff_group][$tariff_group->period])){
                    $tariffPerMonth[$tariff_group->tariff_group][$tariff_group->period] = $tariff_group->total;
                }
                else{
                    $tariffPerMonth[$tariff_group->tariff_group][$tariff_group->period]+=$tariff_group->total;
    ...
    public function getByPeriod($param){
        ...
        if($param == 'VnS'){
            $byPeriod = DB::table('lines_connect')
                ->select(DB::raw("id, MONTHNAME(date_period) as period, sum(duration)as total,
                                case when CAST(date_period as time) between '07:00:00.001' and
                                    '18:00:00.000' then 'Day' else 'Night' end as time_period"))
                ->where('tariff_group', '!=', 'GPRS')
                ->groupBy(DB::raw("MONTHNAME(date_period), case when
    ...
    public function getComparative(){
        ...
        $other_tariff = ['MTN', 'ETISALAT', 'AIRTEL', 'GLO' ];

        $SLA_GPRS_RATE = 150522;
        $SLA_MONTH_DATA = 200; //200GB;
        $COMPARATIVE_MONTH_DATA = 1; //10GB for MTN, AIRTEL et al.

        $tariff_rate = [  ['name'=>'ONNET',  'MTN'=>'0.20', 'ETISALAT'=>'0.12', 'AIRTEL'=>'0.11', 'GLO'=>'0.11' ],
            ['name'=>'LOCAL',  'MTN'=>'0.26', 'ETISALAT'=>'0.9', 'AIRTEL'=>'0.11', 'GLO'=>'0.15' ],
            ['name'=>'ZONE1', 'MTN'=>'0.20', 'ETISALAT'=>'0.5', 'AIRTEL'=>'0.55', 'GLO'=>'0.13' ],
            ['name'=>'GPRS',  'MTN'=>'2500', 'ETISALAT'=>'1000', 'AIRTEL'=>'1500', 'GLO'=>'1000' ]];
            ...
            foreach($tariff_rate as $nRate){
                if($nRate['name'] != $tRate['tariff_group']){
                    ...
                    foreach($other_tariff as $other_tf ){
                        if($nRate['name'] != "GPRS")
                            $comparativeResult[$tRate['tariff_group']][$other_tf] =  $tRate['bill'] / $nRate[$other_tf];
                        else{
                            $comparativeResult[$tRate['tariff_group']][$other_tf] = ($tRate['bill']/ $nRate[$other_tf]) * ($COMPARATIVE_MONTH_DATA * 1024 * 1024 *1024 * 8);
                        }
                    }

                    $comparativeResult[$tRate['tariff_group']]['duration'] = $tRate['duration'];
